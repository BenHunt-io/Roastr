package layout;

/**
 * Created by Ben on 1/13/2017.
 */

class FilteredClass {


    public String[][] bodyRoastArrayDirty ={{"You are so fat that a picture of you fell off the wall",
            "You didn't fall out of the stupid tree. You were dragged through dumbass forest.. you eat too much",
            "You look like a before picture.",
            "You’re so fat, when God said let their be light, he said get your fat ass out the way",
            "I’ve been told that inside every fat person, there’s someone beautiful… I’m just wondering who the hell you ate?",
            "Your ass is so fat, your asshole is mistaken for the blackhole",
            }, {
            "God you're fucking basic as shit.. who want's to be average",
            },{
            "You’ve been invited to the God damn gym’s EMPLOYEE Holiday party before",
            "Rest days? You don't know what a fucking rest day even is!",
            "God Damn, you know the names of ALL the staff BRO",
            "The only thing bigger than his traps is his child porn collection",
            "Do the helicopters land on your forehead? Ya fucking meat head",
            "Working out isnt going to make your dick any bigger bud",
            "Your breathe must smell like Bigfoot's dick",
            "Your primary source of protein comes from the locker room showers"
            }};


    public String[][] bodyRoastArrayDirtyBoth = {{"I'm not saying you're fat, but it looks like you were poured into your clothes and someone forgot to say \"when\"",
            "You're so fat the only letters of the alphabet you know are KFC",
            "You look like a before picture",
            "When was the last time you could see your whole body in the mirror?",
            "You're so fat you need cheat codes to play Wii Fit",
            "You are so fat, everytime she walks she does the harlem shake",
            "You’re so fat, I took a picture of you last Christmas and it’s still printing",
            "By the looks of it, you must be in some kind of fitness protection program?",
            "Everyone stop insulting him, he has enough on his plate already",
            "When you stepped on the scale I thought my phone number popped up",
            }, {
            "Stop being so basic, be fit or be fat! Sheesh",
            }, {
            "When you miss a fitness class or a workout, you genuinely feel a sense of guilt",
            "Almost everyone who works at the gym knows your name, how many siblings you have, your star sign, etc",
            "You take more showers at the gym than in your own apartment",
            "You wonder what exactly people do with their lives when they say things like, “I don’t work out.” No seriously, what do they do?",
            "When people ask you to name a list of things you can’t live without. Your gym membership is in the top 5",
            "You’re late for work because your were working out. Totally worth it",
            "You feel a compulsive need to call everyone in the gym ‘bro’",
            "Your Mail Goes to the Gym",
            "I'm sorry I don't have anything witty here. I am just baffled at how disproportionate this man's body is to his head",
            "You can't deadlift the weight of depression",
            "\"Gym rat\" often used for guys like this who work out a lot but get nowhere",
            "You are definitely a high school hero",
            "You look like a gay Chris Nuñez on steroids",
            }};


    public String[][] intelligenceArrayDirty = {{"You didn't fall out of the stupid tree. You were dragged through dumbass forest",
            "You're about as useless as a vibrator with no batteries",
            "You are the result of 4 billion years of evolutionary success. Fucking act like it",
            } , {
            "Basic ass bitch",
            }, {
            "Hey man you got lots of Good stories, but in what chapter do you shut the fuck up?"
             }};



    public String[][] intelligenceArrayBoth = {{"You're the reason this country has to put directions on shampoo",
            "It's a wonder how some people like you found their way out of the birth canal",
            "You need to act your age not your IQ",
            "I was hoping for a battle of wits in this roast, but you appear to be unarmed",
            "Were you born this stupid or did you take lessons?",
            "If you spoke your mind, you'd be speechless",
            "I'm not saying you're stupid, I'm just saying you've got bad luck when it comes to thinking",
            "I'm sorry I hurt your feelings when I called you stupid. I really thought you already knew",
            "You're as useless as the \"g\" in lasagna",
            "You are depriving some poor village of its idiot",
            "If I ever needed a brain transplant I would pick yours because I want one that hasn't been used yet",
            "You're so stupid, you went to the dentist to get bluetooth",
            "You're so stupid you tried to steal a free sample",
            "You are living proof of reincarnation. No one could possibly get to be so stupid in just one lifetime",
            "Brains aren't everything. In fact, in your case they're nothing",
            "You're proof that evolution can go in reverse",
            "Your village just called. They're missing an idiot",
            "When you go to the mind reader, do you get half price?"
             }, {
            "\"Stop being so basic, be dumb or smart Sheesh\",",
            }, {
            "Intelligence is like an underwear. It is important that you have it, but not necessary that you show it off",
            "You know that tingly little feeling you get when you like someone? That's your common sense leaving your body",
            "If I wanted to kill myself I'd climb your ego and jump to your IQ",
            "Keep talking acting like you're smart, someday you'll say something intelligent!"
            }};


    public String[][] wealthArray = {{"You have too jack off your dog too feed your cat",
            "When you asked your mom what's for dinner she opened her legs and said spaghetti!",
            "Yo mama is so poor, I took a piss on her front lawn and she thanked me for watering the lawn"
            }, {
            "I bet you shop at target because you're basic"
             }, {
            "It's a good thing you're rich. Maybe you can pay someone to take your virginity before you hit 30",
            "no amount of money will make up for that shit between your shoulders you call a head",
            "Look at those birth defects, did your mother scrape you out of the condom and bash you up her sloppy hole? Money won't fix that, Downie",
            "Your money will never make up for having a small penis",

            }};

    public String[][] wealthArrayBoth = {{"If your father didn't cut holes in your pockets at Christmas, you wouldn't have anything to play with",
            "People from the church would run over animals in front of your house to help with food",
            "Beggars give you money",
            "You don't have a pot to piss in, or a window to throw it out of",
            "Someone saw you kicking a can down the street, and when asked what they were doing you said, \"moving\"",
            "You can't afford to pay attention",
            "Your parents got married for the rice",
            "A guy walked into your house, stepped on a cigarette and your mom yelled, \"Who turned off the heat?\"",
            "You live in a 2 story cracker jack box",
            "Someone rang your doorbell and you had to yell \"Ding Dong!\" out the window",
            "You have to fart to get a scent (cent)",
            "You go to KFC and lick other peoples fingers",
            "You buy an imitation of a fake Rolex",
            "You are so poor you ate cereal with a fork to save milk",
            "You are so poor you got a eviction notice on your car",
            "You are so poor you scam Nigerians",
            }, {
            "I bet you shop at target because you're basic"
            }, {
            "Daddy couldn't afford a better nose job?",
            "You're not rich",
            "You aren't rich enough to get away with being that ugly",
            "Money can buy happiness, but it can't buy your smile being fixed",
            "Bill Gates would be ashamed of how poor you are",
            "It's good that he's rich cuz it's gonna take a lot of money to fix that face",
            "The only thing fading faster than that Vineyard vines shirt is daddys money",
            "You know how you see ugly rich guys with stunning women? I don't think you'll ever be THAT rich",
            "You're not a rich kid your a wal-mart manager",
            "Your parents are rich and you've got no intrinsic value",
            "Clearly you're not rich enough for a nose job",
            }};



    public String[][] heightArrayDirty = {{"What's it like having marble sized testicles?",
            "You're so short you have to do 69 with a snorkle"
            }, {

            }, {
            ""
            }};

    public String[][] heightArrayBoth = {{"I wouldn't make fun of your height, I wouldn't stoop that low",
            "You are the eigth, long, forgotten dwarf that no one knows about",
            "You're so short that you would have to stand on a chair to reach puberty",
            "You're so short, you'll need a ladder to reach manhood",
            "You're so short, when it rains you are always the last one to know",
            "You're so short, you can do back flips under your bed",
            "You're so short if you pulled your socks up you'd be blind",
            "You're so short you could bungee jump off a curb",
            "You're so short you would drown by the time you realised it was raining",
            "You're so short that when you sit down on the sidewalk your feet dangle",
            "I need to be careful of what I say here, little guy might just jump up and punch me square in the knee",
            "You are so short you have to look up to see what's going down",
            "It's funny how you're old enough to buy alcohol, but not tall enough to ride the rides as Six Flags",
            "You're so short you can see your feet in your drivers license",
            }, {
            "You're average height.. oh wait that's still short",
            }, {
            "People like you are only good for two things, laughing at them and getting things from the top shelf",
            "Really gonna try and insult somebody for being tall.. really? It's impossible",
            "How do you make a tall person look more awkward? You can’t"
            }};

    public String[][][] dirtySelections = {bodyRoastArrayDirty, intelligenceArrayDirty, wealthArray, heightArrayDirty};
    public String[][][] cleanSelections = {bodyRoastArrayDirtyBoth, intelligenceArrayBoth, wealthArrayBoth, heightArrayBoth};
    public String[][][][] allSelections = {dirtySelections, cleanSelections};
}